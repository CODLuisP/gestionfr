...
export default function Page() {
  ...
  const exportExcel = () => {
    ...
    headersRuta69.forEach((header, index) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const url = `https://villa.velsat.pe:8443/api/Datero/controlEdu/${date}/${selectedRoute}/serfrymh`;
      ...
      const newRows = result.data.map((item: Despacho) => {
        ...
        controlesRuta69.forEach((control) => {
          ...
          <table className="min-w-full border text-sm text-center border-collapse">
            <thead>
              <tr className="bg-slate-200">
                {headersRuta69.map((header, index) => {
                  if (subdividedColumns.includes(index)) {
                    return (
                      <th
                        key={index}
